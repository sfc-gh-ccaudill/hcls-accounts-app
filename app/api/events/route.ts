...
export async function POST(request: Request) {
  try {
    ...
    const { 
      account_id, user_id, use_case_id, title, event_type, location_type, 
    ...
    const result = await query<{ID: number}>(`
      ...
      (ACCOUNT_ID, USER_ID, USE_CASE_ID, TITLE, EVENT_TYPE, LOCATION_TYPE, EVENT_DATE, EVENT_TIME, ATTENDEES, OBJECTIVE, NOTES)
      VALUES (
        ${account_id}, 
        ${user_id || 'NULL'},
        ${use_case_id || 'NULL'}, 
...
export async function PUT(request: Request) {
  try {
    const body = await request.json();
    const { id, use_case_id, title, event_type, location_type, event_date, event_time, attendees, objective, notes } = body;
